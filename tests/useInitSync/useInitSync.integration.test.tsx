...
describe("useInitSync Integration with useStore", () => {
  ...
  describe("Multiple concurrent useInitSync operations", () => {
    test("should handle multiple useInitSync operations affecting different paths", async () => {
      const store = proxy({
        users: null,
        posts: null,
        comments: null,
      ...
      const usersAsyncFn = async (state: typeof store) => {
        await new Promise((resolve) => setTimeout(resolve, 50));
        state.users = [{ id: 1, name: "User 1" }];
        state.metadata.usersLoaded = true;
        return state.users;
      };
      ...
      const postsAsyncFn = async (state: typeof store) => {
        ...
        state.metadata.postsLoaded = true;
        return state.posts;
      };

      const commentsAsyncFn = async (state: typeof store) => {
        await new Promise((resolve) => setTimeout(resolve, 150));
        state.comments = [{ id: 1, text: "Comment 1" }];
        state.metadata.commentsLoaded = true;
        return state.comments;
      };

      function DataLoaders() {
        useInitSync(store, usersAsyncFn);
        useInitSync(store, postsAsyncFn);
        useInitSync(store, commentsAsyncFn);
        return null;
      }

      function UsersDisplay() {
        const users = useStore(store, (s) => s.users);
        const usersLoaded = useStore(store, (s) => s.metadata.usersLoaded);
        return (
          <div data-testid="users-display">
            Users: {users?.length || 0} (loaded: {usersLoaded.toString()})
          </div>
        );
      }
      ...
      function PostsDisplay() {
        ...
        const postsLoaded = useStore(store, (s) => s.metadata.postsLoaded);
        return (
          <div data-testid="posts-display">
            Posts: {posts?.length || 0} (loaded: {postsLoaded.toString()})
          </div>
        );
      }
      ...
      function CommentsDisplay() {
        ...
        const commentsLoaded = useStore(store, (s) => s.metadata.commentsLoaded);
        return (
          <div data-testid="comments-display">
            Comments: {comments?.length || 0} (loaded: {commentsLoaded.toString()})
          </div>
        );
      }
      ...
      function App() {
        return (
          <div>
            ...
            <Suspense fallback={<div data-testid="users-loading">Loading users...</div>}>
              <UsersDisplay />
            </Suspense>
            <Suspense fallback={<div data-testid="posts-loading">Loading posts...</div>}>
              <PostsDisplay />
            </Suspense>
            <Suspense fallback={<div data-testid="comments-loading">Loading comments...</div>}>
              <CommentsDisplay />
            </Suspense>
          </div>
        );
      ...
      // Users should resolve first (50ms)
      await waitFor(() => {
        expect(screen.getByTestId("users-display")).toHaveTextContent("Users: 1 (loaded: true)");
      });

      // Posts and comments should still be loading
      expect(screen.getByTestId("posts-loading")).toBeInTheDocument();
      expect(screen.getByTestId("comments-loading")).toBeInTheDocument();

      // Posts should resolve next (100ms)
      await waitFor(() => {
        expect(screen.getByTestId("posts-display")).toHaveTextContent("Posts: 1 (loaded: true)");
      });

      // Comments should still be loading
      expect(screen.getByTestId("comments-loading")).toBeInTheDocument();

      // Finally comments should resolve (150ms)
      await waitFor(() => {
        expect(screen.getByTestId("comments-display")).toHaveTextContent("Comments: 1 (loaded: true)");
      });
    });

    test("should handle interdependent async operations", async () => {
      const store = proxy({
        user: null,
        userPosts: null,
        postComments: null,
      ...
      const userPostsAsyncFn = async (state: typeof store) => {
        ...
        await new Promise((resolve) => setTimeout(resolve, 50));
        const posts = [
          { id: 1, title: "Post 1", userId: state.user.id },
          { id: 2, title: "Post 2", userId: state.user.id },
        ];
      ...
      const commentsAsyncFn = async (state: typeof store) => {
        ...
        while (!state.userPosts) {
          await new Promise((resolve) => setTimeout(resolve, 10));
        }

        await new Promise((resolve) => setTimeout(resolve, 50));
        const comments = [{ id: 1, postId: state.userPosts[0].id, text: "Great post!" }];
      ...
      function DataLoaders() {
        useInitSync(store, userAsyncFn);
        useInitSync(store, userPostsAsyncFn);
        useInitSync(store, commentsAsyncFn);
        return null;
      }
      ...
      function UserInfo() {
        ...
        return <div data-testid="user-info">User: {user?.name || "Loading..."}</div>;
      }

      function PostsList() {
        const posts = useStore(store, (s) => s.userPosts);
        return <div data-testid="posts-list">Posts: {posts?.length || 0}</div>;
      }

      function CommentsList() {
        const comments = useStore(store, (s) => s.postComments);
        return <div data-testid="comments-list">Comments: {comments?.length || 0}</div>;
      }
      ...
      // Posts should resolve next
      await waitFor(() => {
        expect(screen.getByTestId("posts-list")).toHaveTextContent("Posts: 2");
      });

      // Comments should still be loading
      expect(screen.getByTestId("comments-loading")).toBeInTheDocument();

      // Finally comments should resolve
      await waitFor(() => {
        expect(screen.getByTestId("comments-list")).toHaveTextContent("Comments: 1");
      });
    });
  });

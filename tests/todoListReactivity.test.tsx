import { render, screen, fireEvent, act } from '@testing-library/react';
import { proxy, useStore } from '../src/main';
import { useMemo } from 'react';

// Mock todo interface
interface Todo {
  id: number;
  text: string;
  completed: boolean;
  priority: 'high' | 'medium' | 'low';
  category: string;
}

interface TodoState {
  todos: Todo[];
  filter: 'all' | 'active' | 'completed';
  priorityFilter: 'all' | 'high' | 'medium' | 'low';
  categoryFilter: string;
  searchQuery: string;
}

describe('Todo List Fine-Grained Reactivity', () => {
  let todoState: TodoState;
  let renderCounts = {
    todoList: 0,
    todoItem: 0,
    filterControls: 0,
  };

  beforeEach(() => {
    renderCounts = {
      todoList: 0,
      todoItem: 0,
      filterControls: 0,
    };

    todoState = proxy<TodoState>({
      todos: [
        { id: 1, text: 'Task 1', completed: false, priority: 'high', category: 'Work' },
        { id: 2, text: 'Task 2', completed: true, priority: 'medium', category: 'Personal' },
        { id: 3, text: 'Task 3', completed: false, priority: 'low', category: 'Work' },
      ],
      filter: 'all',
      priorityFilter: 'all',
      categoryFilter: 'all',
      searchQuery: '',
    });
  });

  // Test Component: TodoItem - only subscribes to individual todo
  function TodoItem({ todo }: { todo: Todo }) {
    renderCounts.todoItem++;
    
    const toggleComplete = () => {
      const index = todoState.todos.findIndex(t => t.id === todo.id);
      if (index !== -1) {
        todoState.todos[index].completed = !todoState.todos[index].completed;
      }
    };

    return (
      <div data-testid={`todo-${todo.id}`}>
        <input 
          type="checkbox" 
          checked={todo.completed}
          onChange={toggleComplete}
          data-testid={`checkbox-${todo.id}`}
        />
        <span>{todo.text}</span>
      </div>
    );
  }

  // Test Component: TodoList - PROBLEMATIC approach with object selector
  function ProblematicTodoList() {
    renderCounts.todoList++;
    
    // This creates a new object every time, causing infinite re-renders
    const { todos, filter } = useStore(todoState, (s) => ({
      todos: s.todos,
      filter: s.filter,
    }));

    const filteredTodos = useMemo(() => {
      return todos.filter(todo => {
        if (filter === 'active') return !todo.completed;
        if (filter === 'completed') return todo.completed;
        return true;
      });
    }, [todos, filter]);

    return (
      <div data-testid="todo-list">
        {filteredTodos.map(todo => (
          <TodoItem key={todo.id} todo={todo} />
        ))}
      </div>
    );
  }

  // Test Component: TodoList - CORRECT approach
  function CorrectTodoList() {
    renderCounts.todoList++;
    
    // Subscribe to individual properties
    const todos = useStore(todoState, (s) => s.todos);
    const filter = useStore(todoState, (s) => s.filter);

    const filteredTodos = useMemo(() => {
      return todos.filter(todo => {
        if (filter === 'active') return !todo.completed;
        if (filter === 'completed') return todo.completed;
        return true;
      });
    }, [todos, filter]);

    return (
      <div data-testid="todo-list">
        {filteredTodos.map(todo => (
          <TodoItem key={todo.id} todo={todo} />
        ))}
      </div>
    );
  }

  // Test Component: Filter Controls
  function FilterControls() {
    renderCounts.filterControls++;
    const filter = useStore(todoState, (s) => s.filter);
    
    return (
      <div data-testid="filter-controls">
        <button 
          onClick={() => todoState.filter = 'all'}
          data-testid="filter-all"
        >
          All ({filter === 'all' ? 'active' : 'inactive'})
        </button>
        <button 
          onClick={() => todoState.filter = 'active'}
          data-testid="filter-active"
        >
          Active ({filter === 'active' ? 'active' : 'inactive'})
        </button>
        <button 
          onClick={() => todoState.filter = 'completed'}
          data-testid="filter-completed"
        >
          Completed ({filter === 'completed' ? 'active' : 'inactive'})
        </button>
      </div>
    );
  }

  test('should detect infinite re-renders with object selector', async () => {
    const consoleMock = jest.spyOn(console, 'error').mockImplementation(() => {});
    
    expect(() => {
      render(<ProblematicTodoList />);
    }).toThrow(); // Should throw due to infinite re-renders

    consoleMock.mockRestore();
  });

  test('should handle todo completion changes correctly with proper selectors', async () => {
    render(
      <div>
        <CorrectTodoList />
        <FilterControls />
      </div>
    );

    const initialTodoListRenders = renderCounts.todoList;
    const initialFilterRenders = renderCounts.filterControls;

    // Toggle completion of first todo
    const checkbox1 = screen.getByTestId('checkbox-1');
    
    act(() => {
      fireEvent.click(checkbox1);
    });

    // TodoList should re-render because it needs to recalculate filtered todos
    // when a todo's completed status changes
    expect(renderCounts.todoList).toBeGreaterThan(initialTodoListRenders);
    
    // FilterControls should NOT re-render because filter didn't change
    expect(renderCounts.filterControls).toBe(initialFilterRenders);
    
    // Checkbox should be checked
    expect(checkbox1).toBeChecked();
  });

  test('should handle filter changes correctly', async () => {
    render(
      <div>
        <CorrectTodoList />
        <FilterControls />
      </div>
    );

    const initialTodoListRenders = renderCounts.todoList;
    const initialFilterRenders = renderCounts.filterControls;

    // Change filter to 'active'
    const activeFilterButton = screen.getByTestId('filter-active');
    
    act(() => {
      fireEvent.click(activeFilterButton);
    });

    // Both components should re-render
    expect(renderCounts.todoList).toBeGreaterThan(initialTodoListRenders);
    expect(renderCounts.filterControls).toBeGreaterThan(initialFilterRenders);
    
    // Should only show active todos (id: 1, 3)
    expect(screen.getByTestId('todo-1')).toBeInTheDocument();
    expect(screen.queryByTestId('todo-2')).not.toBeInTheDocument(); // completed todo
    expect(screen.getByTestId('todo-3')).toBeInTheDocument();
  });

  test('should demonstrate the core issue: todo completion affects filtered list', async () => {
    render(<CorrectTodoList />);

    // Set filter to show only active todos
    act(() => {
      todoState.filter = 'active';
    });

    // Should show 2 active todos (id: 1, 3)
    expect(screen.getByTestId('todo-1')).toBeInTheDocument();
    expect(screen.queryByTestId('todo-2')).not.toBeInTheDocument();
    expect(screen.getByTestId('todo-3')).toBeInTheDocument();

    const initialRenderCount = renderCounts.todoList;

    // Complete todo 1 - this should cause TodoList to re-render
    // because the filtered list changes
    const checkbox1 = screen.getByTestId('checkbox-1');
    
    act(() => {
      fireEvent.click(checkbox1);
    });

    // TodoList MUST re-render because:
    // 1. Todo 1 is now completed
    // 2. Filter is 'active' 
    // 3. So Todo 1 should disappear from the filtered list
    expect(renderCounts.todoList).toBeGreaterThan(initialRenderCount);
    
    // Todo 1 should no longer be visible (it's completed and we're showing active)
    expect(screen.queryByTestId('todo-1')).not.toBeInTheDocument();
    expect(screen.queryByTestId('todo-2')).not.toBeInTheDocument();
    expect(screen.getByTestId('todo-3')).toBeInTheDocument();
  });

  test('should NOT re-render TodoList when unrelated state changes', async () => {
    render(<CorrectTodoList />);

    const initialRenderCount = renderCounts.todoList;

    // Change a property that TodoList doesn't subscribe to
    act(() => {
      todoState.searchQuery = 'some search';
    });

    // TodoList should NOT re-render because it doesn't subscribe to searchQuery
    expect(renderCounts.todoList).toBe(initialRenderCount);
  });

  test('should provide correct solution approach', () => {
    // The correct approach for TodoList subscription:
    // 1. Subscribe to individual properties, not objects
    // 2. Ensure that changes to todo.completed are detected
    // 3. Use proper useMemo dependencies
    
    function OptimalTodoList() {
      const todos = useStore(todoState, (s) => s.todos);
      const filter = useStore(todoState, (s) => s.filter);
      
      // The key insight: we need to track changes to individual todo properties
      // This can be achieved by:
      // Option 1: Use a selector that includes the data we care about
      const completionStates = useStore(todoState, (s) => 
        s.todos.map(t => ({ id: t.id, completed: t.completed }))
      );
      
      const filteredTodos = useMemo(() => {
        return todos.filter(todo => {
          if (filter === 'active') return !todo.completed;
          if (filter === 'completed') return todo.completed;
          return true;
        });
      }, [todos, filter, completionStates]);

      return (
        <div>
          {filteredTodos.map(todo => (
            <TodoItem key={todo.id} todo={todo} />
          ))}
        </div>
      );
    }

    // This approach should work without infinite loops
    expect(() => render(<OptimalTodoList />)).not.toThrow();
  });
});
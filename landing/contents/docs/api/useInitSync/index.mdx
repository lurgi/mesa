...
function UserProfile() {
  useInitSync(userStore, async (state) => {
    // ⚠️ All state changes below are batched - intermediate states won't trigger renders
    state.error = null;
    ...
    try {
      ...
      state.initialized = true; // Ensure we exit loading state even on error
    }
    // All changes above are applied atomically when initialization completes
  });

  const { user, initialized, error } = useStore(userStore);
...
function Dashboard() {
  useInitSync(dashboardStore, async (state) => {
    ...
    try {
      ...
      state.error = error.message;
      state.initialized = true; // Ensure we complete initialization
    }
    // Component will only re-render once with all data populated
  });

  const { user, stats, notifications, initialized, error } = useStore(dashboardStore);
...
// Handle initialization states correctly with batching
useInitSync(store, async (state) => {
  ...
  try {
    ...
    state.error = error.message;
    state.initialized = true; // Always complete initialization
  }

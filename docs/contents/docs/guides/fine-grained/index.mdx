...
// Tracks path: "items.length"
const count = useStore(state, s => s.items.length);

// Tracks path: "items.0" (first element)
const first = useStore(state, s => s.items[0]);

// Tracks path: "todos.0.done"
const firstTodoDone = useStore(state, s => s.todos[0]?.done);

// Tracks path: "todos" (entire array)
const allTodos = useStore(state, s => s.todos);
```
...
const user = useStore(state, s => s.user); // Re-renders for any user change
```

...
function ComputedStats() {
  ...
  const computedData = useStore(counterState, (s) => ({
    ...
    average: s.history.length > 0 
      ? s.history.reduce((sum, action) => sum + action.value, 0) / s.history.length 
      : 0
  }));
...
function ConditionalDisplay() {
  ...
  const lastAction = useStore(counterState, (s) => 
    s.history.length > 0 ? s.history[s.history.length - 1] : null
  );
  
  return (
    <div>
      {hasHistory ? (
        <div>Last action: {lastAction?.action}</div>
      ) : (
    ...
    function TestCounter() {
      ...
      return (
        <div>
          ...
          <button onClick={() => testState.count++}>
            Increment
          </button>
        </div>
      );
